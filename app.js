...
function formatMobile(input) {
  ...
  const oldValue = input.value.replace(/[^\d]/g, ''); // Raw digits
  let newValue = oldValue;

  // Handle input (restrict to digits)
  input.addEventListener('input', () => {
    let value = input.value.replace(/[^\d]/g, '');
    if (value.length > 11) value = value.slice(0, 11); // Limit to 11 digits
    if (value && !value.startsWith('03')) {
      value = '03' + value.slice(value.length > 2 ? 2 : 0); // Ensure starts with 03
    }
    ...
    if (value.length > 4 && cursorPos > 4) newCursorPos++; // Account for dash
  ...
  input.addEventListener('paste', (e) => {
    ...
    let value = pasted.slice(0, 11);
    if (value && !value.startsWith('03')) value = '03' + value.slice(value.length > 2 ? 2 : 0);
    if (value.length >= 4) {
      value = `${value.slice(0, 4)}-${value.slice(4, 11)}`;
    }
    input.value = value;
...
function validateMobile(value) {
  // Allow formatted (03xx-xxxxxxx) or raw (03xxxxxxxxx) input
...
window.onload = function () {
  ...
  // Initialize input masks (avoid duplicate listeners)
  const mobileInput = document.getElementById("mobile");
  mobileInput.removeEventListener("input", formatMobile); // Prevent duplicates
  formatMobile(mobileInput);
  ...
  // Initialize profile fields (will be handled in toggleProfileFields)
  document.querySelectorAll("#profileFields input[id='cellNo'], #profileFields input[id='advocateCell']").forEach(input => {
    input.removeEventListener("input", formatMobile); // Prevent duplicates
    formatMobile(input);
  });
...
function toggleProfileFields() {
  ...
  // Apply mobile formatting to cellNo and advocateCell
  document.querySelectorAll("#profileFields input[id='cellNo'], #profileFields input[id='advocateCell']").forEach(input => {
    input.removeEventListener("input", formatMobile); // Prevent duplicates
    formatMobile(input);
  });
